...
definition
  = "Dim" _ path:path _ "As" _ type:("Object" / "String" / "Integer" / "Float" / "Boolean") _ {
    let subType = types;
    for (const key of path.slice(0,-1)) {
    ...
    let lastKey = path[path.length-1];
    subType[lastKey] = type === "Object" ? {} : type;
  }
  ...
  = key:key _? "[" values:(_ / declaration)* "]" _? ";" {
    return (pos, path) => {
      ...
      if (typeof type !== "object") throw new Error(
        type ?
        `${pathToString(path)} is of type "Object" but should be ${JSON.stringify(type)}` :
        `type for ${pathToString(path)} is not defined`
      );
      return [ key, Object.fromEntries(values.filter(v => v).map( (v,i) => v(pos.concat(i+1),path) )) ];
    }
  }
