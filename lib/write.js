...
function encodeValues(val, res, parentIndexes, depth) {
  ...
  for (let i = 0; i < nodes.length;) {
    const node = nodes[i];
    i++; // all uses after this line are 1 based
    if (typeof val[node] !== "object") {
      res.push(`${getDepthIndent(depth)}${parentIndexes.concat(i).join("0.")}0 ${getDepthMarker(depth)} ${val[node]}`);
    } else {
      // We've got an object
      res.push(`${getDepthIndent(depth)}${parentIndexes.concat(i).join("0.")}0 ${getDepthMarker(depth)} <<${node}>> [`);
      encodeValues(val[node], res, parentIndexes.concat(i + 1), depth + 1);
